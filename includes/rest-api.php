...
namespace WSUWP\People_Directory\REST_API;
 ...
 */
function profile_post_type_meta_keys() {
	return \WSUWP\People_Directory\Profile_Post_Type\meta_keys();
}

/**
 * Returns a list of post meta keys to include in REST API responses from secondary sites.
 *
 * @since 0.3.8
 *
 * @return array
 */
function profile_post_type_secondary_meta_keys() {
	return array(
...
add_filter( 'rest_authentication_errors', __NAMESPACE__ . '\\rest_verify_authentication', 90 );
...
add_action( 'init', __NAMESPACE__ . '\\show_university_taxonomies_in_rest', 12 );
...
add_filter( 'rest_' . profile_post_type_slug() . '_collection_params', __NAMESPACE__ . '\\rest_collection_params' );

add_action( 'rest_api_init', __NAMESPACE__ . '\\profile_propagation' );

if ( false === \WSUWP\People_Directory\is_primary() ) {
	add_action( 'rest_api_init', __NAMESPACE__ . '\\register_secondary_api_fields' );
}
 ...
 */
function rest_verify_authentication( $result ) {
	global $wp;
	...
	// Check the nonce.
	$result = \WSUWP\People_Directory\verify_rest_nonce( $nonce, $uid, $domain );
 ...
 * @since 0.3.0
 */
function custom_access_control_headers() {
	remove_filter( 'rest_pre_serve_request', 'rest_send_cors_headers' );
	add_filter( 'rest_pre_serve_request', __NAMESPACE__ . '\\rest_send_cors_headers' );
}
...
/**
 * Registers the custom meta fields for a REST API response containing profiles.
 *
 * @since 0.2.0
 */
function register_api_fields() {
	$args = array(
		'get_callback' => __NAMESPACE__ . '\\get_api_meta_data',
	);

	foreach ( profile_post_type_meta_keys() as $field_name => $value ) {
		if ( isset( $value['updatable_via_rest'] ) ) {
			$args['update_callback'] = __NAMESPACE__ . '\\update_api_meta_data';
			$args['schema'] = rest_field_schema( $value );
		}

		register_rest_field( profile_post_type_slug(), $field_name, $args );
	}

	register_rest_field( profile_post_type_slug(), 'taxonomy_terms', array(
		'get_callback' => __NAMESPACE__ . '\\get_api_taxonomy_data',
		'update_callback' => __NAMESPACE__ . '\\update_api_taxonomy_data',
		'schema' => array(
...
/**
 * Returns the value of a post meta field sanitized against a whitelist
 ...
 */
function get_api_meta_data( $object, $field_name, $request ) {
	if ( ! array_key_exists( $field_name, profile_post_type_meta_keys() ) ) {
		return '';
	}
	...
	if ( 'esc_url_raw' === profile_post_type_meta_keys()[ $field_name ]['sanitize_callback'] ) {
		return esc_url( get_post_meta( $object['id'], profile_post_type_meta_keys()[ $field_name ]['meta_key'], true ) );
	}
...
/**
 * Update the value of a post meta field sanitized against a whitelist
 * with the provided method.
 *
 * @since 0.2.0
 *
 ...
 * @return mixed
 */
function update_api_meta_data( $value, $object, $field_name ) {
	if ( ! array_key_exists( $field_name, profile_post_type_meta_keys() ) ) {
		return;
	}
...
/**
 * Sets up the schema for each registered meta field.
 *
 * @since 0.3.0
 *
 * @param array @args
 ...
 * @return array
 */
function rest_field_schema( $args ) {
	$schema = array(
...
/**
 * Returns an array of term names and slugs keyed by taxonomy name.
 *
 * @since 0.3.0
 *
 * @param array           $object     The current post being processed.
 ...
 */
function get_api_taxonomy_data( $object, $field_name, $request ) {
...
/**
 * Updates the taxonomy terms associated with a post.
 *
 * @since 0.3.15
 *
 ...
 * @return mixed
 */
function update_api_taxonomy_data( $value, $object, $field_name ) {
	if ( ! $field_name && ! $value ) {
		return;
	}
...
/**
 * Exposes the University taxonomies in the REST API.
 *
 * @since 0.3.0
 */
function show_university_taxonomies_in_rest() {
 ...
 * @since 0.2.2
 */
function register_wsu_nid_query_var() {
	global $wp;
 ...
 * @return array
 */
function rest_query_vars( $valid_vars, $request ) {
...
/**
 * Sets a meta query for WSU NID when the wsu_nid parameter is included as
 * part of a query.
 *
 * @since 0.2.2
 *
 ...
 */
function handle_wsu_nid_query_var( $query ) {
 ...
 * @return array
 */
function rest_collection_params( $query_params ) {
...
function profile_propagation() {
	register_rest_route( 'wsuwp-people/v1', '/sync', array(
		...
		'callback' => __NAMESPACE__ . '\\update_primary_profile_listing_data',
		'args' => array(
			'ids' => array(
	...
	register_rest_route( 'wsuwp-people/v1', '/sync/(?P<nid>[\w.]+)', array(
		'methods' => \WP_REST_Server::EDITABLE,
		'callback' => __NAMESPACE__ . '\\update_profile_terms',
		'args' => array(
			'nid' => array(
				'validate_callback' => function( $param, $request, $key ) {
 ...
 */
function update_primary_profile_listing_data( $request ) {
 ...
 * @return WP_Error|WP_REST_Request
 */
function update_profile_terms( $request ) {
	...
	if ( $posts ) {
		$update_terms = update_api_taxonomy_data( $taxonomy_terms, $posts[0], 'taxonomy_terms' );
...
/**
 * Registers secondary custom meta fields for a REST API response containing profile data.
 *
 * @since 0.3.8
 */
function register_secondary_api_fields() {
	$args = array(
		'get_callback' => __NAMESPACE__ . '\\get_secondary_api_meta_data',
	);

	foreach ( profile_post_type_secondary_meta_keys() as $rest_key => $field_name ) {
		register_rest_field( profile_post_type_slug(), $rest_key, $args );
	}
}
 ...
 * @return mixed Meta data associated with the post and field name.
 */
function get_secondary_api_meta_data( $object, $rest_key ) {
	if ( ! array_key_exists( $rest_key, profile_post_type_secondary_meta_keys() ) ) {
		return '';
	}

	return esc_html( get_post_meta( $object['id'], profile_post_type_secondary_meta_keys()[ $rest_key ], true ) );
}
